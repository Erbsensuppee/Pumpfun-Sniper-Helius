...
export let bundleSignature;

export async function createJitoBundle(transaction, wallet, connection) {
  try {
    ...
    bundleSignature = bs58.encode(transaction.signatures[0]);
...
export async function sendJitoBundle(bundle) {
  try {
    const response = await axios.post(
      JITO_RPC_URL,
      {
